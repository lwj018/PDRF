...
def scheduled_mix_extrap_with_gt(
    extrapolated: torch.Tensor,
    ...
    """
    在训练中将部分半拉格朗日外推帧替换为真实帧（scheduled）
    extrapolated: [B, T_out, ...] 半拉格朗日外推
    x_start:      [B, T_out, ...] 真实未来
    epoch:        当前 epoch (0-based)
    total_epochs: 总 epoch 数
    p_max:        第 0 个 epoch 时，替换为真实帧的概率
    p_min:        最后一个 epoch 时的替换概率（一般设为 0）
    """

    assert extrapolated.shape == x_start.shape
    device = extrapolated.device

    # 线性衰减的 teacher forcing 概率：从 p_max -> p_min
    progress = float(epoch) / max(total_epochs - 1, 1)
    p_gt = p_max * (1.0 - progress) + p_min * progress  # 早期大，后期趋近 0

    if p_gt <= 0.0:
        # 不再替换，直接用纯外推
        return extrapolated

    B, T = extrapolated.shape[0], extrapolated.shape[1]
    # 构造 [B, T, 1, 1, ...] 形式的掩码，保证后面维度可 broadcast
    extra_dims = [1] * (extrapolated.dim() - 2)  # 针对 H, W, C 等
    mask_shape = [B, T] + extra_dims

    # True 表示使用真实帧，False 表示使用外推帧
    use_gt = (torch.rand(mask_shape, device=device) < p_gt)

    mixed = torch.where(use_gt, x_start, extrapolated)
    return mixed
...
class Runner(object):
    ...
    def __init__(self, args):
        ...
        self.extrapolator = PySTEPSExtrapolationWrapper(
    ...
    def _preparation(self):
        ...
        self.exp_dir    = osp.join(cur_dir, 'Exps_x_icml_review4_3', self.args.exp_dir, self.exp_name)        
    ...
    def _build_model(self):
        ...
        elif self.args.backbone == 'ours':
            model = UKan_Hybrid(T=1000,in_ch=self.args.frames_out,out_ch=self.args.frames_out, ch=64, ch_mult=[1, 2, 2, 2], attn=[],num_res_blocks=2, dropout=0.1).to(device)
    ...
    def _train_batch(self, batch):
        ...
        if hasattr(self.model, 'module'):
            extrapolated_condition, target_frames = self.extrapolator.prepare_training_data(
                    frames_in, frames_out)
            extrapolated_condition=extrapolated_condition.squeeze(2)
            loss_dict = rf.training_losses(model=self.model,x_start=frames_out,condition=extrapolated_condition)
            loss = loss_dict["loss"]
        else:
            #_, loss = self.model.predict(frames_in=frames_in, frames_gt=frames_out, compute_loss=True)
            condition=frames_in.squeeze(2)
            x_start=frames_out.squeeze(2)
            x_sl = self.extrapolator.get_extrapolated_condition(condition, x_start.shape[1]).detach()
            #extrapolated_condition2 = self.extrapolator.build_condition(condition, extrapolated_condition)
            loss_dict = rf.training_losses(model=self.model,x_start=x_start,condition=condition,x_sl=x_sl)
            loss = loss_dict["loss"]
        if loss is None:
    ...
    def _sample_batch(self, batch, use_ema=False, vis_diff=False):
        ...
        z = torch.randn(radar_input.shape[0],self.args.frames_out,128,128,requires_grad=False).to(device)
        condition=radar_input.squeeze(2)
        extrapolated_condition = self.extrapolator.get_extrapolated_condition(condition,self.args.frames_out).detach()
        extrapolated_condition2 = self.extrapolator.build_condition(condition, extrapolated_condition)
        pred = rf.sampleno(self.model,z,condition,device = device)
        radar_pred=pred.unsqueeze(2)
        #radar_pred=extrapolated_condition.unsqueeze(2)
...
#/media/ps/code_data/SEVIR/data/sevir/data
#python run.py --eval --visual --ckpt_milestone   resources/ckpt-best.pt