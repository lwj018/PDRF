...
class SemiLagrangianExtrapolation:
    """
    半拉格朗日外推（改良版）
...
class PySTEPSExtrapolationWrapper:
    """
    外推包装器：对接你现有训练/推理流程
    - get_extrapolated_condition: 生成 Y_base（K 帧）
    - prepare_training_data: (Y_base, frames_out)
    - build_condition: 拼装 cond = concat(ctx, Y_base)
    """

    def __init__(
        self,
        extrapolation_timestep: int = 5,
        **kwargs
    ):
        self.extrapolator = SemiLagrangianExtrapolation(
            motion_estimation_method=kwargs.get("motion_estimation_method", "farneback"),
            extrapolation_timestep=extrapolation_timestep,
            border_treatment=kwargs.get("border_treatment", "nearest"),
            interp_order=kwargs.get("interp_order", 3),
            smooth_flow_sigma=kwargs.get("smooth_flow_sigma", 1.2),
            clip_flow_ratio=kwargs.get("clip_flow_ratio", 0.10),
            refresh_flow_every=kwargs.get("refresh_flow_every", None),
        )
    ...
    def get_extrapolated_condition(self, frames_in: torch.Tensor, frames_out_len: int) -> torch.Tensor:
        ...
        """
        return self.extrapolator.extrapolate_tensor(frames_in, frames_out_len)
