...
class UKan_Hybrid(nn.Module):
    def __init__(self, T,in_ch,out_ch, ch, ch_mult, attn, num_res_blocks, dropout):
        ...
        self.CGSTF3=ConditionGuidedSpatialTransformFusion(channels=128)
        self.CGSTF4=ConditionGuidedSpatialTransformFusion(channels=256)
        self.CGSTF5=ConditionGuidedSpatialTransformFusion(channels=320)



        self.free1=FreeFusion(128,0.9,1.2)
        self.free2=FreeFusion(256,0.2,1.4)
...
if __name__ == '__main__':
    ...
    x=torch.rand(4,5,128,128).cuda()
    y=torch.rand(4,20,128,128).cuda()
    t = torch.rand(x.size(0)).cuda()
    # flops, params = profile(model, inputs=(x,t,y))
    out=model(y,t,x)
    print(out.shape)
